...
int main(int argc, const char * argv[]) {
    ...
    // 1. 先z创建一个日历对象。日历对象的作用可以从日期对象中取到日期的各个部分。
    NSCalendar *calendar = [NSCalendar currentCalendar];
    // 2. 让日历对象从日期对象中取出日期的各个部分
    NSDateComponents *com = [calendar components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitWeekday | NSCalendarUnitHour fromDate: date1];
    ...
    // 1. 先创建一个日期格式化对象，这个对象可以将一个日期输出为指定的格式
    ...
    //    mm   分钟
    //    ss   秒
    formatter1.dateFormat = @"yy-MM-dd hh:mm:ss";
    // 3. 然后格式化对象按照指定的格式将日期对象转换。
    //    转换的时候回自动的转换为当前系统的时区的时间。
    NSString *str2 = [formatter1 stringFromDate:date5];
    NSLog(@"%@", str2);
    ...
    // 3. 转换。
    NSDate *date6 = [formatter2 dateFromString:strDate];
    NSLog(@"%@", date6);
